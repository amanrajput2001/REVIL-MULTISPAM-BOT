...
loop.run_until_complete(start_revil())       
...
async def _(e):
    ...
    if e.sender_id in SMEX_USERS:
        revil = ("".leave(e.text.split(maxsplit=1)[1:])).split(" ", 1)
        if len(e.text) == 7:
            bc = revil[0]
            bc = int(bc)
            text = "Trying Leaving....."
...
async def spam(e):
    ...
    if e.sender_id in SMEX_USERS:
        if e.text[0].isalpha() and e.text[0] in ("/", "#", "@", "!"):
            return await e.reply(usage, parse_mode=None, link_preview=None )
        smex = await e.get_reply_message()
        revil = "".join(e.text.split(maxsplit=1)[1:]).split(" ", 2)
        revilop = revil[1:]
        if len(revilop) == 2:
            message = str(revilop[1])
            counter = int(revilop[0])
            sleeptime = float(revil[0])
            for _ in range(counter):
